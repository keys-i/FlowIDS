...
0 = M0 Base, 1 = Small, 2 = Matched. Each job requests one
...
The array has no concurrency cap: all 3 may run together if GPUs and account
limits allow. Do not add `%2` to `--array` if you want all 3 running at once.
3 GPUs for three hours use up to 9 GPU-hours; 72 hours allows 216 GPU-hours.
