...
def run(config: Config, *, evaluate_only: bool = False, hours: float = 3) -> None:
    ...
    if evaluate_only:
        ...
        pretraining = config.model.kind in {"reconstruct", "teacher"}
        ...
        if pretraining:
            ...
            objective = Pretrainer(model, config.model.kind).to(device)
            loaders = {
                name: make_loader(dataset, config, shuffle=name == "train", device=device)
            ...
            history = pretrain(
                ...
                config,
                device,
                deadline=perf_counter() + (training_deadline - perf_counter()) / 2,
            )
            torch.save(
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        usage="%(prog)s [max] {M0,M1} variant [--seed SEED] [--evaluate-only]",
    )
    _ = parser.add_argument("model", choices=("M0", "M1"))
